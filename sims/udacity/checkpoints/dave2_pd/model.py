...
def build_model(num_outputs: int = NUM_OUTPUTS) -> tf.keras.Model: # type: ignore
    ...
    model = models.Model(inputs=img_in, outputs=out, name="dave2_baseline")
    opt = optimizers.Adam(learning_rate=LEARNING_RATE)

    model.compile(optimizer=opt, loss="mse", metrics=[tf.keras.metrics.MeanAbsoluteError(name="acc")]) # PD used "acc" in examples, but that's not meaningful for regression. Use MAE.
    return model
