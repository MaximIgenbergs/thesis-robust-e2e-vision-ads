...
Output Structure
----------------
<runs_base>/udacity/<map>/<test_type>/<model>/<YYYYmmdd_HHMMSS>__<tag>/
  ├── manifest.json           # run-level metadata + episode index
  ├── config_snapshot.json    # paths/roads/perturbations/run (+ host/port if provided)
  ├── env_snapshot.txt        # pip freeze
  └── episodes/
      └── 0001/
          ├── meta.json       # episode metadata
          └── pd_log.json     # PerturbationDrive / env log
"""
...
def _default_runs_base() -> Path:
    ...
    """
    # file is .../sims/udacity/logging/eval_runs.py => go up 4 levels to repo root
...
def make_runs_root(
    ...
    """
    base = Path(runs_base) if runs_base is not None else _default_runs_base()
    root = base / "udacity" / map_name / test_type / model_name
...
def prepare_run_dir(
    ...
    """
    runs_root = make_runs_root(map_name=map_name, test_type=test_type, model_name=model_name, runs_base=runs_base)
...
class RunLogger:
    ...
    Manage a run directory with:
      - manifest.json (run-level metadata + episode index)
      - config_snapshot.json (paths/roads/perturbations/run[/host_port])
      - env_snapshot.txt (pip freeze)
    ...
    def snapshot_configs(
        ...
        cfg_host_port: Optional[Dict[str, Any]] = None,
    ) -> None:
        snap = {
            "paths": {
                "sim_app": str(sim_app),
        ...
        if cfg_host_port is not None:
            snap["configs"]["host_port"] = cfg_host_port
