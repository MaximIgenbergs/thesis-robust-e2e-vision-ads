...
def main():
    ...
    if not train_files:
        raise SystemExit(f"[dave2] No training images found for glob: {INPUTS_GLOB}")

    run_dir = make_run_dir(model_key="dave2", map_name=MAP_NAME)
...
if __name__ == "__main__":
    # Allow dynamic GPU memory growth if GPU is present
    try:
