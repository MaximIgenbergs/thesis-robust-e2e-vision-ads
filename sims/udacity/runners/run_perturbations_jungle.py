#!/usr/bin/env python3
# Jungle map robustness runner for the legacy Udacity simulator.
# Parity with sims/udacity/runners/run_perturbations.py (chunk -> pert -> severity),
# with minimal differences: legacy UdacityGym comms + soft resets (no process reconnects).
...
# --- configs & logging (same as PD runner) ---
from sims.udacity.configs import paths, roads, perturbations, run
from sims.udacity.utils.logging import (
    RunLogger, make_run_dir, make_run_id,
    module_public_dict, best_effort_git_sha, pip_freeze
)

# --- legacy simulator stack ---
...
# --- PD image perturbation (loads masks in __init__ ONLY for funcs passed) ---
...
def _force_start_episode(env: UdacityGym, track="jungle", weather="sunny", daytime="day", timeout_s=30.0):
    """
    Robustly enter Jungle without restarting the Unity process.
    ...
    while True:
        ...
        if now - last_emit > 1.0:
            ...
            last_emit = now
        if now - t0 > timeout_s:
            raise RuntimeError("Failed to enter Jungle before timeout.")
        time.sleep(0.2)
...
def main() -> int:
    # --- paths & adapter (PD parity) ---
    sim_app = _abs(getattr(paths, "SIM", getattr(paths, "PD_SIM", "")))
    ...
    logger = RunLogger(
        ...
        sim_name="udacity_legacy_jungle",
        git_info=git_info,
    )
    logger.snapshot_configs(
        ...
        cfg_roads=module_public_dict(roads),          # parity with PD runner
    ...
    severities = list(getattr(run, "SEVERITIES", [0, 3, 5]))
    ...
    # --- initial boot (single Unity process for all runs) ---
    sim = UdacitySimulator(sim_exe_path=str(sim_app), host=run.HOST, port=run.PORT)
